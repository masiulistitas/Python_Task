...
def get_user_average_age(x):
    my_age = [i['age'] for i in x if 'age' in i]
    average = sum(my_age) / len(my_age)
    print('Users average age:', average)
...
def get_user_names(x):
    my_names = [i['name'] for i in x if 'name' in i]
    print('All users names in list alphabetically ordered:', sorted(my_names))
